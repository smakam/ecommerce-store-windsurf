  ...
  passport.authenticate('google', { session: false }, (err, user, info) => {
    ...
    if (!user) {
      ...
      if (origin && /https:\/\/ecommerce-store-windsurf[-.a-z0-9]+-srees-projects-ef0574fa\.vercel\.app/.test(origin)) {
        ...
        errorRedirectUrl = `${previewUrl}/oauth-handler.html?error=auth_failed&source=google_oauth_error&time=${Date.now()}`;
        console.log('Using preview URL for error redirect to static HTML handler:', errorRedirectUrl);
      } 
      // Check if the request is coming from localhost
      else if (origin && origin.includes('localhost')) {
        errorRedirectUrl = `http://localhost:3000/oauth-handler.html?error=auth_failed&source=google_oauth_error&time=${Date.now()}`;
        console.log('Using localhost for error redirect to static HTML handler:', errorRedirectUrl);
      }
      // Default to the main production URL
      else {
        errorRedirectUrl = `${process.env.FRONTEND_URL}/oauth-handler.html?error=auth_failed&source=google_oauth_error&time=${Date.now()}`;
        console.log('Using production URL for error redirect to static HTML handler:', errorRedirectUrl);
      }
    ...
    if (origin && /https:\/\/ecommerce-store-windsurf[-.a-z0-9]+-srees-projects-ef0574fa\.vercel\.app/.test(origin)) {
      ...
      // Use static HTML handler
      redirectUrl = `${previewUrl}/oauth-handler.html?token=${token}&source=google_oauth&time=${Date.now()}`;
      console.log('Using preview URL for redirect to static HTML handler:', redirectUrl);
    } 
    // Check if the request is coming from localhost
    else if (origin && origin.includes('localhost')) {
      redirectUrl = `http://localhost:3000/oauth-handler.html?token=${token}&source=google_oauth&time=${Date.now()}`;
      console.log('Using localhost for redirect to static HTML handler:', redirectUrl);
    }
    // Default to the main production URL
    else {
      redirectUrl = `${process.env.FRONTEND_URL}/oauth-handler.html?token=${token}&source=google_oauth&time=${Date.now()}`;
      console.log('Using production URL for redirect to static HTML handler:', redirectUrl);
    }
